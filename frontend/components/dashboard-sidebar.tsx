...
export default function DashboardSidebar(props: Props) {
  ...
  const [currentMusic, setCurrentMusic] = useState<Music | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    ApiService.getPauseState(props.guildId).then((res) =>
      setPauseState(res.data),
    );

    const onQueueUpdated = (curMusic: Music | undefined, curQueue: Music[]) => {
      setQueue(curQueue);
      setCurrentMusic(curMusic);
    };
    const onPauseStateUpdated = (pause: boolean) => setPauseState(pause);

    WSService.wsEvents.on("queue_updated", onQueueUpdated);
    WSService.wsEvents.on("pause_state_updated", onPauseStateUpdated);

    return () => {
      WSService.wsEvents.off("queue_updated", onQueueUpdated);
      WSService.wsEvents.off("pause_state_updated", onPauseStateUpdated);
    };
  }, [props.guildId]);
      ...
      <CardContent className="flex flex-col flex-1 gap-4 p-2 overflow-auto">
        <div className="flex flex-col gap-2 items-center">
          ...
          <div className="flex gap-2 w-full">
            <Button className="flex-1" onClick={updatePauseState}>
        ...
        <div className="flex flex-col gap-2 overflow-auto text-gray-500">
          {currentMusic ? (
            <div className="flex gap-2 border-1 rounded-sm p-2 hover:bg-accent transition-colors text-foreground">
              <ChevronRight width={18} />
              <MusicIcon width={18} />
              <span className="truncate max-w-[300px]">
                {decodeURI(currentMusic.name)}
              </span>{" "}
            </div>
          ) : undefined}
          {queue.map((music, index) => (
            <div
              key={index}
              className="flex gap-2 border-1 rounded-sm p-2 hover:bg-accent transition-colors"
            >
              <MusicIcon width={18} />
              <span className="truncate max-w-[300px]">
                {decodeURI(music.name)}
              </span>{" "}
            </div>
          ))}
        </div>
