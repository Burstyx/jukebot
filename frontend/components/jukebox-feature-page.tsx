...
export default function JukeboxFeaturePage(props: Props) {
  ...
  const [filteredMusics, setFilteredMusics] = useState<Music[] | undefined>(
    musics,
  );

  const filterMusics = useCallback((query: string) => {
    if (!musics) return;

    const normalizedQuery = query.trim().toLowerCase();

    if (normalizedQuery === "") return setFilteredMusics(musics);
  ...
  useEffect(() => {
    ...
    const onMusicAdded = (music: Music) =>
      setMusics((prev) => [...prev, music]);
    const onMusicRemoved = (music: Music) =>
      setMusics((prev) => prev.filter((m) => m.hash !== music.hash));
