...
export const getChannels = async (req: FastifyRequest<{ Params: { guild_id: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
...
export const getSelectedChannel = async (req: FastifyRequest<{ Params: { guild_id: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
    const guildId = req.params.guild_id;
...
export const getMusics = async (req: FastifyRequest<{ Params: { guild_id: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
    const guildId = req.params.guild_id;
...
export const addToQueue = async (req: FastifyRequest<{ Params: { guild_id: string, music_hash: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
    const guildId = req.params.guild_id;
    ...
    if (!guildId)
        return reply.status(400).send(Errors.createError("Certaines informations sont manquantes.", { guildId, musicHash }))
...
export const getQueue = async (req: FastifyRequest<{ Params: { guild_id: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
    const guildId = req.params.guild_id;
...
export const getPauseState = async (req: FastifyRequest<{ Params: { guild_id: string } }>, reply: FastifyReply, jukebot: Jukebot) => {
    const guildId = req.params.guild_id;
