...
export const uploadFile = async (
    ...
    return reply
      ...
      .send(
        Errors.createError("Certaines informations sont manquantes.", {
          guildId,
          dataSpecified: rawData !== undefined,
          fileName: fileName,
        }),
      );
  ...
  if (!Buffer.isBuffer(rawData))
    return reply
      .status(400)
      .send(Errors.createError("Les données reçu ne sont pas valides."));

  if (await LibraryService.exists(guildId, rawData, true))
    return reply
      .status(400)
      .send(
        Errors.createError("Ce fichier audio existe déjà pour ce serveur."),
      );

  try {
    const newMusic = await LibraryService.uploadFile(
      rawData,
      guildId,
      fileName,
      "Inconnu",
